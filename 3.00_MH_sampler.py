...
mh_samples = np.memmap(filename = mem_file_1, dtype='float32', mode='w+', shape=(num_chains, num_samples)) # CVAR training data memmap
...
for i in range(num_chains):
    ...
    for j in range(round(num_samples/iter_size)):
        ...
        mh_samples[i, j*iter_size:(j+1)*iter_size] = np.array(distances)

        init_val = samples[-1]

        print(np.mean(distances))

print(f"Time:{(timeit.default_timer() - start)/60}")
