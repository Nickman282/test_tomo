...
mem_file_1 = Path('D:/Studies/MEng_Project/Lung_CT/raw_results/unet_90_training.mymemmap') 
...
torch.save(model, os.path.join(os.getcwd(), "unet/unet_fbp_90.pt"))

unet_90_training = np.memmap(filename = mem_file_1, dtype='float32', mode='w+', shape=(num_batches*epochs, 1)) # CVAR training data memmap
unet_90_training[:] = np.stack(losses_list)[:]