...
class UNet(nn.Module):
    ...
    UNet module
    """
    def __init__(self, device, image_size, theta, channel_in=1, channels=32):
        super().__init__()
        ...
        self.radon = FastRadonTransform(image_size=image_size, theta=theta)

        self.noise = torch.randn(image_size).to(device)
    ...
    def loss_fn(self, sino):
        ...
        recon_sino = self.radon(recon_x)

        lamb = 0.01
        bs_img, c_img, h_img, w_img = recon_x.size()

        ## Compute Loss
        # Reconstruction loss function
        loss = F.mse_loss(sino, recon_sino, reduction='sum')
        tv_h = torch.pow(recon_x[:,:,1:,:]-recon_x[:,:,:-1,:], 2).sum()
        tv_w = torch.pow(recon_x[:,:,:,1:]-recon_x[:,:,:,:-1], 2).sum()
        loss += lamb*torch.sqrt(tv_h+tv_w)/(bs_img*c_img*h_img*w_img)
