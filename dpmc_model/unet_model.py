...
# Group normalization with 32 groups of total channels
class GroupNorm32(nn.GroupNorm):
    def forward(self, x):
        return super().forward(x.float()).type(x.dtype)

# Basic timestep building blocks
...
class Upsample(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.dims == 3:
            ...
            # 2D interpolation along HxW dims, 4x feature map size
...
class Downsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, dims=2, out_channels=None):
        ...
        if use_conv:
            ...
            # Downsample 4x using average pooling
...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        # Perform group normalization, then activation,
        # then 3x3 conv with optional channel depth change
        ...
        # Timestep embedding for scale_shift_norm
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        in_channels = 1,
        model_channels = 128,
        out_channels = 1, # lookup "learn_sigma", may need 2
        num_res_blocks = 1,
        ...
        num_classes = None,
        use_checkpoint = True,
        use_fp16 = False, # Turn on if insufficient memory?
        num_heads = 4,
        ...
        # Time embeddings have 4x dimensionality of channel depth
        time_embed_dim = model_channels * 4

        # Time embedding is performed on the first ResNet block?
        ...
        # Num channels at the first ResNet block
        ch = input_ch = int(channel_mult[0] * model_channels)

        # Add timestep embedding to the input block
        ...
        ds = 1
        # For each channel depth (level*128) defined in "level" tuple
        for level, mult in enumerate(channel_mult):
            # Define n residual blocks per ResNet layer
